...
def dashboard_operativo(request):
    ...
    juntas_programadas = Junta.objects.select_related("comite").order_by("fecha")[:10]
...
def generar_registros_junta(request, junta_id):
    ...
    nuevos = [
        AsistenciaJunta(junta=junta, ciudadano_id=cid, asistio=False)
        for cid in ciudadanos_ids
        if cid not in existentes_ids
    ]
    ...
    messages.success(request, f"Se generaron {len(nuevos)} registros de asistencia pendientes para la junta '{junta.tema}'.")
    return redirect("dashboard_operativo")
