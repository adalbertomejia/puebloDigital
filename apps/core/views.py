...
def dashboard_operativo(request):
    ...
    context = {
        ...
        "quick_links": {
            ...
            "faenas_operativas": reverse("faenas_operativas"),
...
@require_POST
def generar_participantes_faena(request, pk):
    faena = get_object_or_404(Faena, pk=pk)
    if faena.estado == Faena.Estados.CERRADA:
        messages.error(request, "La faena está cerrada. No se pueden generar participantes.")
    ...
    if not ciudadanos_ids:
        messages.warning(request, "No hay ciudadanos activos para generar participantes.")
        return redirect("faena_operativa", pk=pk)

    existentes = set(RegistroFaena.objects.filter(faena=faena).values_list("ciudadano_id", flat=True))
    nuevos = [
        RegistroFaena(
            faena=faena,
            ciudadano_id=ciudadano_id,
            estado=RegistroFaena.EstadosAsistencia.PENDIENTE,
        )
    ...
    messages.success(request, f"Generación completada. Se crearon {len(nuevos)} registros nuevos.")
    return redirect("faena_operativa", pk=pk)
...
def faenas_operativas(request):
    estado = request.GET.get("estado", "")
    qs = Faena.objects.select_related("comite").annotate(
    ...
    return render(request, "dashboard/faenas_operativas.html", {
        "faenas": qs[:40],
        "estado_actual": estado,
        "estados_faena": Faena.Estados.choices,
    })
